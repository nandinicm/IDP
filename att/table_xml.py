...
def clean_tables(tables_data):
    ...
    for enum, each_page in enumerate(tables_data):
        for each_table in each_page['tables']:
            ...
            for each_row in each_table['tableRows']:
                ...
                for each_cell in each_row['cells']:
                    # print(each_cell)
                    # print(each_cell['id'])
                    row_data['col' + each_cell['id'][-1]] = each_cell['value']
...
def get_table_data_xml(tables_data):
...
def combine_json_parse_xml(uploadpath):
    ...
    for f in listdir(path):
        with open(uploadpath + "tables/" + f) as ev:
            read_tables.append(ast.literal_eval(ev.read()))
    ...
    xml_op = get_table_data_xml(read_tables)
    print(xml_op)
    ...
    with open(str(uploadpath) + "/" + "data.xml", "wb") as fs:
        fs.write(xml_bytes)
