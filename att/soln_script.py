...
def get_page_hypothesis(image, evidence, rules):
    ...
    # print("Traceback------------------------------------------", tb)
    # with open('/home/devops/logs.txt', 'a') as f:
    # f.write(str(tb))
    # f.write('--------------------------------------------------------------------------')
...
def is_inside(rect1, rect2):  # (x1, y1, x1b, y1b), (x2, y2, x2b, y2b)):
    ...
    if top or left or right or bottom:
        # print('IS INSIDE IS FALSE')
        return False
    else:  # rectangles intersect
        # print('IS INSIDE IS TRUE')
        return True


def mark_positions(image):
    position_clause = {
        'top-half': [0, 0, int(image.shape[0] / 2), image.shape[1]],
        'bottom-half': [int(image.shape[0] / 2), 0, int(image.shape[0]), image.shape[1]],
        'left-half': [0, 0, int(image.shape[0]), int(image.shape[1] / 2)],
        'right-half': [0, int(image.shape[1] / 2), int(image.shape[0]), int(image.shape[1])],
        'top-left-corner': [0, 0, int(image.shape[0] / 2), int(image.shape[1] / 2)],
        'top-right-corner': [0, int(image.shape[1] / 2), int(image.shape[0] / 2), int(image.shape[1])],
        'bottom-left-corner': [int(image.shape[0] / 2), 0, int(image.shape[0]), int(image.shape[1] / 2)],
        'bottom-right-corner': [int(image.shape[0] / 2), int(image.shape[1] / 2), int(image.shape[0]),
                                int(image.shape[1])],
        'all': [0, 0, image.shape[0], image.shape[1]]
    }
